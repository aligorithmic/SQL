...
SELECT rule_template -> '$parameters' -> 'NOMINAL_THICKNESS' ->> 'path'
...
SELECT rule_template -> '$parameters' ? 'NOMINAL_THICKNESS'
FROM graph1.rule_templates;
...
SELECT id, jsonb_extract_path(rule_template, '$parameters', 'NOMINAL_THICKNESS', 'path')
FROM graph1.rule_templates;
